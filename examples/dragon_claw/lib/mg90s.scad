// -----------------------------------------------------------------------------
// Rough model of an MG90S servo motor with convenience functions
...
shaft_len = 0.5; // blank length before spline

wire_drop = 4; // distance from base to wire

// --- Spline dimensions (as printed and tested) ---
mg90s_spline_r1 = (4.2 / 2) + (0.1 * -5);
mg90s_spline_r2 = (4.7 / 2) + (0.1 * 7);
mg90s_spline_n = 20;
mg90s_spline_h = 5; // usable spline length
mg90s_spline_bolt_dia = 2.5; // actually M2.5
...
module spline_shaft(
  ...
  translate([0, 0, -fudge])
    linear_extrude(height=h + fudge) spline_sect(r1, r2, n);
  translate([0, 0, -shaft_len])
    cylinder(r=r1, h=shaft_len);
}
...
function mg90s_wire_size(
  t = mg90s_wire_thickness(),
  w = mg90s_wire_width()
) =
  [t, w];
...
module mg90s_wire_stub(wire_size = mg90s_wire_size()) {
  ...
  translate(mg90s_wire_pos(wire_size))
    cube([wire_thickness, wire_width, wire_thickness], center=true);
}

module mg90s_wire_channel(
  wire_size = mg90s_wire_size(),
  clearance = 0.2,
  wrap_under = false
) {
  let (
  ...
  minkowski() {
    ...
    union() {
      hull() {
        ...
        translate([0, 0, body_height - wire_drop - wire_thickness])
          mg90s_wire_stub();
      }
      if (wrap_under) {
        hull() {
          translate([0, 0, -wire_drop - wire_thickness]) mg90s_wire_stub();
          translate([-body_length, 0, -wire_drop - wire_thickness])
            mg90s_wire_stub();
        }
      }
    }
...
function mg90s_base_center_pos() =
  mg90s_base_pos() + ([body_width - body_length, 0, 0] / 2);
function mg90s_body_center_pos() =
  mg90s_base_center_pos() + [0, 0, body_height / 2];

// Bolt positions
function mg90s_bolt_top_pos() =
  [
    for (mirr = [1, -1]) mg90s_base_center_pos() + [mirr * wing_bolt_spacing / 2, 0, body_height - wing_drop],
  ];

function mg90s_bolt_top_angles() = [[0, 0, 0], [0, 0, 180]];

function mg90s_bolt_bottom_pos() =
  [for (pos = mg90s_bolt_top_pos()) pos - [0, 0, wing_thick]];

function mg90s_bolt_bottom_angles() =
  [for (a = mg90s_bolt_top_angles()) [180, 0, 0] + a];
...
module mg90s(a = 0) {
  ...
  // body with wings and bolt holes
  difference() {
    translate(mg90s_base_center_pos()) {
      translate([0, 0, body_height / 2])
        cube(body_dims, center=true);
      translate([0, 0, body_height - wing_drop - wing_thick / 2])
        cube(wing_dims, center=true);
    }
    for (i = [0, 1]) {
      translate(mg90s_bolt_top_pos()[i])
        rotate(mg90s_bolt_top_angles()[i]) {
          cylinder(d=wing_bolt_dia, h=wing_thick * 3, center=true);
          translate([(wing_len - wing_bolt_spacing) / 4, 0, 0])
            cube(
              [
                (wing_len - wing_bolt_spacing) / 2,
                wing_split,
                wing_thick * 3,
              ], center=true
            );
  ...
  translate(mg90s_shaft_pos())
    rotate(mg90s_shaft_axis())
      rotate([0, 0, a])
        spline_shaft();
}

// use with slice_height = -mg90s_wire_pos()[2] to include wire
module mg90s_section(
  clearance = 0.1,
  slice_height = -body_height / 2,
  use_hull = false
) {
  offset(r=clearance) {
    if (use_hull) {
      hull() projection(cut=true)
          translate([0, 0, -slice_height]) mg90s();
    } else {
      projection(cut=true)
        translate([0, 0, -slice_height]) mg90s();
    }
